...
// 마스터 시트에서 학생 링크를 삭제(퇴원)하는 액션
export async function deleteLinkFromMasterSheetAction(targetUrl) {
  const scriptUrl = process.env.APPS_SCRIPT_URL;
  ...
  if (!targetUrl) {
    return { success: false, message: '삭제할 URL이 제공되지 않았습니다.' };
  }
  ...
  try {
    ...
    if (data.result === 'success') {
      // 캐시를 강제로 삭제하여 즉시 반영되게 만듦
      revalidatePath('/');
      revalidatePath('/settings');
      return { success: true };
    } else {
      return { success: false, message: data.message || '삭제 중 오류가 발생했습니다.' };
    }
  } catch (error) {
    console.error('Failed to delete link from master sheet:', error);
    return { success: false, message: '네트워크 또는 서버 오류가 발생했습니다.' };
  }
}

// 마스터 시트에서 학생 링크를 복구하는 액션
export async function restoreLinkFromMasterSheetAction(targetUrl) {
  const scriptUrl = process.env.APPS_SCRIPT_URL;
  ...
  if (!targetUrl) {
    return { success: false, message: '복구할 URL이 제공되지 않았습니다.' };
  }
  ...
  try {
    ...
    if (data.result === 'success') {
      ...
      return { success: false, message: data.message || '복구 중 오류가 발생했습니다.' };
    }
  } catch (error) {
    console.error('Failed to restore link from master sheet:', error);
    return { success: false, message: '네트워크 또는 서버 오류가 발생했습니다.' };
  }
}
