...
function parseName(str) {
  ...
  // 1. "English (Korean)" or "English(Korean)"
  let match = str.match(/^([a-zA-Z\s]+?)\s*\(\s*([가-힣]+)\s*\)$/);
  if (match) {
  ...
  // 3. Just Korean or unknown
  // If it's pure english, maybe just put it in kor for now to show something, or put in eng.
  // Actually, let's check if it's purely english.
  if (/^[a-zA-Z\s]+$/.test(str.trim())) {
    return { eng: str.trim(), kor: '' };
  }
